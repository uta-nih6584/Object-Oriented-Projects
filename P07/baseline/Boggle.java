...
public class Boggle {
    ...
    private static void solveRange(int boardIndex, List<String> wordsToSolve, int threadNumber) {
        Solver solver = new Solver(boards.get(boardIndex), threadNumber, verbosity);
        for (String word : wordsToSolve) {
    ...
    public static void main(String[] args) {
        try {
            ...
            int wordsPerThread = (int)Math.ceil((double)words.size() / numThreads);

            for (int boardIndex = 0; boardIndex < numberOfBoards; boardIndex++) {
                for (int i = 0; i < numThreads; i++) {
                    int start = i * wordsPerThread;
                    int end = Math.min(start + wordsPerThread, words.size());
                    List<String> wordsToSolve = words.subList(start, end);

                    if (!wordsToSolve.isEmpty()) {
                        final int threadNumber = i; // Effectively final for the callable
                        futures.add(executor.submit(() -> solveRange(boardIndex, wordsToSolve, threadNumber)));
                    }
                }
            }
